...
- Orçamento de render: `composer test:thermometer-budget` valida teto de consultas (10) e de bytes de HTML (1,55 MB) para a grade de 12 meses; os dados de cada linha vêm prontos do `ThermometerGrid`. As medições de tempo do Termômetro são sempre relativas (mesmo instrumento antes/depois): o FPM ainda paga o boot do app por requisição, medido em ~110 ms de PHP num redirect trivial pelo SPX — número obtido com profiler ativo, portanto a confirmar antes de qualquer decisão de custo fixo.
...
- Custo de interação: abrir o detalhe do dia e o "adicionar" já é barato (o Filament devolve apenas o modal, sem re-render da grade); check-in e navegação de período re-renderizam os 12 meses (medido: dados ~33 ms + render ~71 ms = 102–106 ms, resposta ~1,36 MB). Islands do Livewire não servem para isolar por mês (não podem ficar dentro de um laço, não enxergam variáveis do template e o modal é desenhado fora da ilha); a alternativa de marcar o check-in no cliente foi descartada por decisão de produto. O refactor com componente Livewire por mês também foi avaliado e descartado: ao trocar de mês todas as props mudam e os 12 meses re-renderizam do mesmo jeito, ou seja, a navegação não melhora; o único beneficiário seria o check-in, que já foi recusado, e o custo seria reescrever a estrutura da página e as asserções que leem a grade pelo presenter.
- Handlers da grade: as células chamam métodos finos da página (`openCell(date, type)` e `addMovement(date, type)`), que delegam para `mountAction` preservando a otimização de partial do Filament (abrir o dia devolve só o modal, ~30 ms). O `budget` trava o payload da página em 1,55 MB (hoje ~1,47 MB).

...
@endphp<td class="thermometer-value-cell"><div class="thermometer-cell-inner"><button type="button" class="thermometer-value-button" data-filled="{{ $isFilled ? 'true' : 'false' }}" wire:click="openCell('{{ $dateKey }}','{{ $type->value }}')" aria-label="Ver {{ mb_strtolower($type->columnLabel()) }} de {{ $shortDay }}, total {{ $money }}{{ $isProjected ? ', com projeções' : '' }}"><span class="thermometer-badge" data-type="{{ $type->value }}" data-filled="{{ $isFilled ? 'true' : 'false' }}" aria-hidden="true">{!! $badge !!}</span><span @class(['thermometer-value', 'thermometer-value-projection' => $isProjected])>{{ $money }}</span></button><button type="button" class="thermometer-add-button" wire:click="addMovement('{{ $dateKey }}','{{ $type->value }}')" aria-label="Adicionar {{ mb_strtolower($type->label()) }} em {{ $shortDay }}">+</button></div></td>@endforeach<td class="thermometer-balance-cell" data-balance-color="{{ $balanceColor }}"><button type="button" class="thermometer-balance-button" wire:click="openCell('{{ $dateKey }}')" aria-label="Saldo do dia {{ $dayLabel }}: {{ $balance }}">{{ $balance }}</button></td></tr>
        @endforeach
    </tbody>

@php
    ...
    $balanceColor = $this->balanceColor($day['balance']);
...
<tr wire:key="thermometer-day-{{ $dateKey }}" @class(['thermometer-row', 'thermometer-row-today' => $day['is_today'], 'thermometer-row-weekend' => $date->isWeekend()])><th scope="row" class="thermometer-day-cell">@if ($day['is_future'])<span class="thermometer-day-static" title="{{ $dayLabel }}">{{ $day['day'] }}</span>@else<button type="button" class="thermometer-day-button" wire:click="toggleCheckIn('{{ $dateKey }}')" aria-pressed="{{ $isCheckedIn ? 'true' : 'false' }}" aria-label="Alternar check-in do dia {{ $dayLabel }}"><span>{{ $day['day'] }}</span>{!! $checkMark !!}</button>@endif</th>@foreach ($this->columns as $type)@php
    ...
    $badge = $badgeSymbol !== null
        ...
        : $badgeLetter;
@endphp@if ($type->isForecast())<td class="thermometer-value-cell"><div class="thermometer-cell-inner"><span class="thermometer-value-static" data-filled="{{ $isFilled ? 'true' : 'false' }}" title="Previsão de {{ $shortDay }}: {{ $money }}"><span class="thermometer-badge" data-type="{{ $type->value }}" data-filled="{{ $isFilled ? 'true' : 'false' }}" aria-hidden="true">{!! $badge !!}</span><span class="thermometer-value thermometer-value-projection">{{ $money }}</span></span></div></td>@else<td class="thermometer-value-cell"><div class="thermometer-cell-inner"><button type="button" class="thermometer-value-button" data-filled="{{ $isFilled ? 'true' : 'false' }}" wire:click="mountAction('openCell', { date: '{{ $dateKey }}', type: '{{ $type->value }}' })" aria-label="Ver {{ mb_strtolower($type->columnLabel()) }} de {{ $shortDay }}, total {{ $money }}{{ $isPending ? ', com projeções' : '' }}"><span class="thermometer-badge" data-type="{{ $type->value }}" data-filled="{{ $isFilled ? 'true' : 'false' }}" aria-hidden="true">{!! $badge !!}</span><span @class(['thermometer-value', 'thermometer-value-projection' => $isPending])>{{ $money }}</span></button><button type="button" class="thermometer-add-button" wire:click="mountAction('addMovement', { date: '{{ $dateKey }}', type: '{{ $type->value }}' })" aria-label="Adicionar {{ mb_strtolower($type->label()) }} em {{ $shortDay }}"><span aria-hidden="true">+</span></button></div></td>@endif@endforeach<td class="thermometer-balance-cell" data-balance-color="{{ $balanceColor }}"><button type="button" class="thermometer-balance-button" wire:click="mountAction('openCell', { date: '{{ $dateKey }}' })" aria-label="Saldo do dia {{ $dayLabel }}: {{ $balance }}">{{ $balance }}</button></td></tr>
